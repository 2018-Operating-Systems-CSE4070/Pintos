...
fixed32_t fraction = 1 << 14;
fixed32_t half_fraction = 1 << 13;
fixed32_t load_avg;
...
int fixed2int(fixed32_t x)
{
    ...
    else return (x + half_fraction) / fraction;
}
...
thread_tick (void) 
{
  ...
  {
    int i;
    if(t != idle_thread) t->recent_cpu = fixed_add_int(t->recent_cpu, 1);
    if(timer_ticks() % TIMER_FREQ == 0)
    {
      struct thread *t2;
      struct list_elem *e;
      fixed32_t c1, c2, r, la2;
      int ready_threads = 0;
      fixed32_t c59, r1, r2, rd;
